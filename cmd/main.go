...
<body>
	...
	v1.StaticFS("/files", http.Dir("/mnt/nas"))
	...
	master.GET("/volunteers/districts", volunteerHandler.GetDistricts)
	...
	volunteerEvents.GET(":id/participation", eventHandler.GetDetailEventsByVolunteer)
	...
	speaks.POST("/:id/attachments/:type", speakHandler.CreateAttachment)
	speaks.DELETE("/:id/attachments/:attachmentId", speakHandler.DeleteAttachment)
	speaks.GET("/:id/attachments/:type", speakHandler.GetAttachments)
	speaks.POST("/:id/action", speakHandler.Action)
