...
type CreateRequest struct {
	...
	AreaID             *int    `json:"area_id" validate:"omitempty,min=1"`
	DonationCategoryID int     `json:"donation_category_id" validate:"required,min=1"`
	Type               int     `json:"type" validate:"oneof=0 1"` // 0 = money, 1 = goods
	Currency           string  `json:"currency" validate:"required,max=10"`
	Amount             float64 `json:"amount" validate:"required_if=Type 0,gte=0"`        // required for money; forced to 0 for goods
	OtherItems         string  `json:"other_items" validate:"required_if=Type 1,max=500"` // required for goods; nulled for money
}
