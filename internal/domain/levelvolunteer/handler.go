...
func (h *Handler) Create(c *gin.Context) {
	...
	if err != nil {
		response.AbortError(c, apperror.Wrap(err, http.StatusInternalServerError, "DB_301", "failed to create level volunteer"))
		return
	}
...
func (h *Handler) GetAll(c *gin.Context) {
	...
	if err != nil {
		response.AbortError(c, apperror.Wrap(err, http.StatusInternalServerError, "DB_302", "failed to fetch level volunteer"))
		return
	}
...
func (h *Handler) GetByID(c *gin.Context) {
	...
	if err != nil {
		response.AbortError(c, apperror.Wrap(err, http.StatusNotFound, "DB_303", "level volunteer not found"))
		return
	}
...
func (h *Handler) Update(c *gin.Context) {
	...
	if err != nil {
		response.AbortError(c, apperror.Wrap(err, http.StatusInternalServerError, "DB_304", "failed to update level volunteer"))
		return
	}
...
func (h *Handler) Delete(c *gin.Context) {
	...
	if err := h.Service.SoftDelete(c.Request.Context(), id, actor); err != nil {
		response.AbortError(c, apperror.Wrap(err, http.StatusInternalServerError, "DB_305", "failed to delete level volunteer"))
		return
	}

	response.Success(c, gin.H{"message": "level volunteer deleted"})
}
